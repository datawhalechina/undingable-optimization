...
&emsp;&emsp;秩一更新（SR1）公式是结构最简单的拟牛顿矩阵更新公式。设$B^k$是第$k$步的近似海瑟矩阵，我们通过对$B^k$进行秩一修正得到$B^{k+1}$，使其满足割线方程$(2.3)$。为此使用待定系数法求出修正矩阵，并设：
$$
...
$$
H^{k+1}=(V^k)^TH^kV^k + \rho_ks^k(s^k)^T \tag {3.4.1}
$$
其中
$$
